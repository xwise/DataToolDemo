...
@Component({
    selector: 'jhi-baseline-anc',
    templateUrl: './baseline-anc.component.html'
})
export class BaselineAncComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.baselineService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<BaselineAnc[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/baseline-anc'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/baseline-anc', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BaselineAnc) {
        return item.id;
    }
