package dk.siemens.enginnering.datatool.service;
...
import dk.siemens.enginnering.datatool.domain.*; // for static metamodels
import dk.siemens.enginnering.datatool.repository.FileEntryRepository;
import dk.siemens.enginnering.datatool.service.dto.FileEntryCriteria;

import dk.siemens.enginnering.datatool.service.dto.FileEntryDTO;
import dk.siemens.enginnering.datatool.service.mapper.FileEntryMapper;
import dk.siemens.enginnering.datatool.domain.enumeration.UploadStatus;
...
public class FileEntryQueryService extends QueryService<FileEntry> {
    ...
    private Specifications<FileEntry> createSpecification(FileEntryCriteria criteria) {
        ...
        if (criteria != null) {
            ...
            if (criteria.getFileNameVersion() != null) {
                specification = specification.and(buildStringSpecification(criteria.getFileNameVersion(), FileEntry_.fileNameVersion));
            }
            if (criteria.getFileName() != null) {
                specification = specification.and(buildStringSpecification(criteria.getFileName(), FileEntry_.fileName));
            }
            if (criteria.getVersion() != null) {
                specification = specification.and(buildStringSpecification(criteria.getVersion(), FileEntry_.version));
            }
            if (criteria.getUploadStatus() != null) {
                specification = specification.and(buildSpecification(criteria.getUploadStatus(), FileEntry_.uploadStatus));
            }
            if (criteria.getBaselineId() != null) {
                specification = specification.and(buildReferringEntitySpecification(criteria.getBaselineId(), FileEntry_.baselines, Baseline_.id));
            }
        }
